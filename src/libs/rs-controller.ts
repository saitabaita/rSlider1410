...
class rsController{
    ...
    constructor(model: rsModel, view: rsView){
        ...
        let pos: number = this.getPositionFromValue(model.value);
        this.view.setPositionView(pos);
        this.view.$control.find('input:text').val(this.model.value+''); 
        let _this = this;
    ...
    updateSlider(){
        this.view.maxHandlePos = this.view.$range[0]['offsetWidth'] - this.view.$control[0]['offsetWidth'];
        let pos: number = this.getPositionFromValue(this.model.value);
        this.view.setPositionView(pos);
    }
    getValueFromPosition(pos: number){
        let percentage: number = ((pos) / (this.view.maxHandlePos || 1));
    ...
    getPositionFromValue(value: number){
        ...
        let pos: number = (!Number.isNaN(percentage)) ? percentage * this.view.maxHandlePos : 0;
    ...
    handleDown(e: any){
        ...
        let value: number = this.getValueFromPosition(this.inrange(pos, 0, this.view.maxHandlePos));
        let newPos: number = this.getPositionFromValue(value);
//        console.log('pos - '+pos+'this.view.maxHandlePos: '+this.view.maxHandlePos+' value: '+value+' newPos: '+newPos)
